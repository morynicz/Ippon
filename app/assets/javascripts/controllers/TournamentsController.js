angular.module('ippon').controller('TournamentsController',[
  '$scope',
  '$stateParams',
  '$location',
  '$resource',
  ...
  function($scope, $stateParams, $location, $resource, $state, Auth, FlashingService){
      ...
      var groupsResource = $resource("tournaments/:tournamentId/groups",
              {
          tournamentId: "@id",
          format: "json"
              });
      ...
      var playoffFightsResource = $resource("tournaments/:tournamentId/playoff_fights",
              {
          tournamentId: "@id",
          format: "json"
              });
      
      var participantsResource = $resource("tournaments/:tournamentId/participants",
              {
          tournamentId: "@id",
          format: "json"
              });

      var groupFightsResource = $resource("tournaments/:groupId/group_fights",
              {
          tournamentId: "@id",
          format: "json"
              });
      ...
      var getTournament = function(tournamentId, next) {
          tournamentResource.get({tournamentId: tournamentId},
                  function(response) {
      ...
      var getGroups = function(tournamentId, next) {
          groupsResource.query({tournamentId: tournamentId}, function(response){
              $scope.groups = response;
          }, function(httpResponse){});
      }
      
      var getTeams = function(tournamentId, next) {
          teamsResource.query({tournamentId: tournamentId}, function(response){
      ...
      var getPlayoffFights = function(tournamentId, next) {
          playoffFightsResource.query({tournamentId: tournamentId}, function(response){
      ...
      var buildLeveledPlayoffTree = function(trunk) {
          var levelArray = [];
          var level = 0;
//          levelArray[0].push(trunk);
          if(null != trunk) {
              buildLevelOfPlayoffTree(trunk, levelArray, level);
          }
//          buildLevelOfPlayoffTree(trunk.playoff_fight.previous_shiro_fight, levelArray, 1);
//          buildLevelOfPlayoffTree(trunk.playoff_fight.previous_aka_fight, levelArray, 1);
          return levelArray;
      }
      ...
      var buildLevelOfPlayoffTree = function(trunk, levelArray, level) {
          ...
          if(null != trunk.playoff_fight.previous_shiro_fight) {
              buildLevelOfPlayoffTree(trunk.playoff_fight.previous_shiro_fight, levelArray, level + 1);
          }
          
          if(null != trunk.playoff_fight.previous_aka_fight) {
              buildLevelOfPlayoffTree(trunk.playoff_fight.previous_aka_fight, levelArray, level + 1);
          }
      }

      var getParticipants = function(tournamentId, next) {
          participantsResource.get({tournamentId: tournamentId}, function(response){
      ...
      getTeams($stateParams.tournamentId, buildTeamsMap);
      getParticipants($stateParams.tournamentId);
      getPlayoffFights($stateParams.tournamentId, buildPlayoffTree);
}]);