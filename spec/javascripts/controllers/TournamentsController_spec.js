describe('TournamentsController',
        function() {
            ...
            var setupController = function(stateName, tournamentId) {
                return inject(function($location, $stateParams, $rootScope,
                        $resource, $httpBackend, $controller, $state,
                        $templateCache) {
                    scope = $rootScope.$new();
                    location = $location;
                    resource = $resource;
            ...
            var expectGetGroups = function(tournamentId, isAdmin, groups) {
                var request = new RegExp("tournaments/" + tournamentId
                        + "/groups");
                var response = groups;

                httpBackend.expectGET(request).respond(200, response);
            }

            var expectGetPlayoffFights = function(tournamentId, isAdmin,
                    playoffFights) {
                var request = new RegExp("tournaments/" + tournamentId
                        + "/playoff_fights");
                var response = playoffFights;

                httpBackend.expectGET(request).respond(200, response);
            }

            var expectGetTeams = function(tournamentId, isAdmin, teams) {
                var request = new RegExp("tournaments/" + tournamentId
                        + "/teams");
                var response = teams;
            ...
            var expectGetParticipants = function(tournamentId, isAdmin,
                    participants) {
                var request = new RegExp("tournaments/" + tournamentId
                        + "/participants");
                var response = {
            ...
            var expectGetTournament = function(tournamentId, isAdmin,
                    tournament) {
                var request = new RegExp("tournaments/" + tournamentId);
                tournament.is_admin = isAdmin;
            ...
            describe('show', function() {
                describe('tournament is found', function() {
                    beforeEach(function() {
                        ...
                        expectGetTournament(fakeTournamentId, false,
                                fakeTournament);
                        expectGetGroups(fakeTournamentId, false, fakeGroups);
                        expectGetTeams(fakeTournamentId, false, fakeTeams);
                        expectGetParticipants(fakeTournamentId, false,
                                fakeParticipants);
                        expectGetPlayoffFights(fakeTournamentId, false,
                                fakePlayoffFights);
                    });
                    it('loads the given tournament', function() {
                        httpBackend.flush();
                        expect(scope.tournament).toEqualData(fakeTournament);
                        expect(scope.playoff_fights).toEqualData(
                                fakePlayoffFights);
                        expect(scope.groups).toEqualData(fakeGroups);
                        expect(scope.teams).toEqualData(fakeTeams);
                        expect(scope.participants)
                                .toEqualData(fakeParticipants);
                ...
                describe('tournament is not found', function() {
                    beforeEach(function() {
                        setupController('tournaments_show', fakeTeamId);
                        expectGetTournamentNotFound(fakeTeamId);
                    });
                    it("doesn't load a tournament", function() {
                        httpBackend.flush();
                        expect(scope.tournament).toEqualData(null);
                        expect(scope.playoffFights).toEqualData(null);
